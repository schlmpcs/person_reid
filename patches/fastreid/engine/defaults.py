...
class DefaultTrainer(TrainerBase):
    ...
    def build_optimizer(cls, cfg, model):
        ...
        """
        # contiguous=False: ContiguousParams' pre-wired grad views are broken by
        # torch>=2.0 zero_grad(set_to_none=True), which nulls per-param grads and
        # leaves the contiguous buffer's .grad None (AMP scaler then sees no grads).
